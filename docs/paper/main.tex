...
\subsection{定量的評価結果}
予備実験において、Adaptive CFGの減衰パラメータ $p$ (Power) を変化させた際の結果を以下に示す。
\begin{itemize}
    \item \textbf{$p=2.0$ (High Stiffness)}: LPIPSは極めて低い（安定）が、CLIP Scoreも低迷した。これはモデルが動作を拒否し、静的な状態（Robot Dancing）に留まったことを示す。
    \item \textbf{$p=0.7$ (Balanced)}: LPIPSはわずかに上昇したが、CLIP Scoreが大幅に向上した。これは、物理的な整合性をある程度犠牲にしつつ、プロンプトのアクション指示を受け入れたことを示唆しており、我々が目指す「動的生成」の成功例である。
\end{itemize}
\end{document}
