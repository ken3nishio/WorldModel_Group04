...
なお、表中の ``Prob.'' は VideoMAE によって予測された Top Class の確信度（Probability）を表している。
...
\subsubsection{Temporal Blur ($\sigma_{blur}$) の役割と効果}
本手法の第3の要素である Temporal Blur は、Self-Attention機構におけるKey/Valueマップに対して、時間軸方向のGaussian Blur ($\sigma_{blur}=0.6$) を適用するものである。
この処理の意図は、モデルが保持する「過去の形状に対する過剰な記憶」を緩和することにある。
通常、Static Deathに陥るモデルは、過去のフレーム（初期の立ち姿など）の特徴量を鮮明に記憶し続けており、これが新しい動作生成を阻害する「アンカー」として機能してしまう。
Blurを導入することで、この時間的な特徴量マップが平滑化され、モデルは厳密な過去の形状を参照できなくなる。
結果として、モデルの過去への執着（Inertia）が物理的に弱まり、Relaxationによって誘発された新しい動作（バク転）への遷移がスムーズに行われるようになる。
また、この「ぼかし」効果は、フレーム間の急激な変化を抑制するローパスフィルタとしても機能するため、生成される動画の時間的連続性を保ち、フリッカー等のアーティファクトを防ぐ効果も確認された。
...
\begin{figure}[t]
  \centering
  \includegraphics[width=0.95\linewidth]{figures/energy_landscape.png}
  \caption{エネルギー地形（Energy Landscape）の比較。(a) \textbf{Baseline (Static Death)}: モデルは初期状態に対応する深い極小解に捕われている。(b) \textbf{Boost (Forced)}: 過剰な力を加えることで地形がカオス化し、構造崩壊を招く。(c) \textbf{Relaxation (Ours)}: ポテンシャルの谷を平坦化することで、モデルはターゲットとなる動作状態へスムーズに遷移できる。}
